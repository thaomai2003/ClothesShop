...
namespace MyFinalExam.Helpers
{
    public sealed class PaypalClient
    {
        ...
        private async Task<AuthResponse> Authenticate()
        {
            ...
            //Tạo HTTP POST request đến v1/oauth2/token để lấy token xác thực.
            ...
            //Đọc và deserializes JSON response thành AuthResponse.
        ...
        public async Task<CreateOrderResponse> CreateOrder(string value, string currency, string reference)
        {
            ...
            //Gửi HTTP POST request đến v2/checkout/orders để tạo đơn hàng.
            var httpResponse = await httpClient.PostAsJsonAsync($"{BaseUrl}/v2/checkout/orders", request);
            var jsonResponse = await httpResponse.Content.ReadAsStringAsync();
            //Đọc và deserializes JSON response thành CreateOrderResponse.
        ...
        public async Task<CaptureOrderResponse> CaptureOrder(string orderId)
        {
            ...
            //Gửi HTTP POST request đến v2/checkout/orders/{orderId}/capture để bắt giữ đơn hàng
            var httpResponse = await httpClient.PostAsync($"{BaseUrl}/v2/checkout/orders/{orderId}/capture", httpContent);
            //Đọc và deserializes JSON response thành CaptureOrderResponse.
            var jsonResponse = await httpResponse.Content.ReadAsStringAsync();
    ...
    //Lưu trữ phản hồi xác thực từ PayPal.
    ...
    // phản hồi từ yêu cầu tạo đơn hàng
